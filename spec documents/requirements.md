...
# Requirements 5_0712_Hyebin
- 아래 기능을 추가해주세요.
* 각 카드(card bevel)의 본문(card-text)이 200자 이상이거나 한 카드 내의 본문 개수가 2개 이상일 때, 200자 분량 이후부터는 해당 지점에 위치한 'More' 버튼으로 이후 텍스트를 축약하는 기능을 추가해주세요. 'MORE'을 클릭하면 나머지 내용이 보이며, 가장 마지막에 'LESS'가 붙습니다. 'LESS'를 누르면 다시 축약된 형태로 돌아갑니다.
* 이벤트 카테고리를 설명하는 카드(예: Market)(card bevel card--section)은 해당 축약 기능을 적용받지 않도록 해주세요.
* 축약되는 텍스트가 20자 이내, 즉 전체 단락의 총 텍스트 길이가 220자이내라면 More/Less 기능이 적용되지 않도록 해주세요.
